...
@interface ttrFriendsListVw()<UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
{
    CGFloat _rowHeight;
    CGSize _keyBoardSize;
}

@property (nonatomic,strong) UITableView * friendsTV;
@property (nonatomic,strong) UISearchBar * streamSearchBar;

@end

@implementation ttrFriendsListVw
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.friendsTV)
        return;
    self.friendsTV = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    self.friendsTV.translatesAutoresizingMaskIntoConstraints = NO;
    [self addSubview:self.friendsTV];
    self.friendsTV.dataSource = self;
    self.friendsTV.delegate = self;
    [self.friendsTV setSeparatorColor:[UIColor clearColor]];
    [self.friendsTV setSeparatorStyle:UITableViewCellSeparatorStyleSingleLine];
    [self.friendsTV setBackgroundColor:[UIColor clearColor]];
    self.friendsTV.contentInset = UIEdgeInsetsZero;
    [self addConstraints:@[[NSLayoutConstraint constraintWithItem:self.friendsTV attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeWidth multiplier:1.0 constant:1.0],[NSLayoutConstraint constraintWithItem:self.friendsTV attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeHeight multiplier:1.0 constant:(-44.0)],[NSLayoutConstraint constraintWithItem:self.friendsTV attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterX multiplier:1.0 constant:0.0],[NSLayoutConstraint constraintWithItem:self.friendsTV attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:(-22.0)]]];
...
{
    return [self.handlerDelegate getNumberOfFriendsForUser];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSString * l_reqdcellid = @"friend_cell";
    ttrFriendsListVwCustomCell * l_newcell = [tableView dequeueReusableCellWithIdentifier:l_reqdcellid];
    if (!l_newcell)
        l_newcell = [[ttrFriendsListVwCustomCell alloc]
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar
{
    ...
    searchBar.text = nil;
    [self.handlerDelegate lookUpFriendsForSearchStr:@""];
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if (!l_searchstr)
        l_searchstr = @"";
    [searchBar resignFirstResponder];
    [self.handlerDelegate lookUpFriendsForSearchStr:l_searchstr];
}

@end

@interface ttrFriendsListVwCustomCell()
{
    NSInteger _posnNo;
    id<ttrFriendsListViewDelegate> _handlerDelegate;
...
@implementation ttrFriendsListVwCustomCell
...
- (void) displayValues
{
    if (_profileView)
    {
        NSDictionary * l_frienddict = [_handlerDelegate getFriendDictOfUserAtPosn:_posnNo];
